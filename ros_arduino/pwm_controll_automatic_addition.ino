...
ros::Publisher rightPub("front_right_ticks", &right_wheel_tick_count);


 
std_msgs::Int16 left_wheel_tick_count;
ros::Publisher leftPub("front_left_ticks", &left_wheel_tick_count);
...
const int motorRightPWM_pin = 9;  // PWM pin for Motor 1
const int motorRightDir_pin = 8;  // Direction pin for Motor 1
const int motorLeftPWM_pin = 10; // PWM pin for Motor 2
const int motorLeftDir_pin = 11; // Direction pin for Motor 2
 
// How much the PWM value can change each cycle
const int PWM_INCREMENT = 1;

const int TICKS_PER_REVOLUTION = 8123;

const double WHEEL_RADIUS = 0.0485;
 
// Distance from center of the left tire to the center of the right tire in m
const double WHEEL_BASE = 0.43;
 
const double TICKS_PER_METER = 26600; // Originally 2880
...
void set_pwm_values() {
  ...
  if (pwmLeftReq > 0) { // Left wheel forward
    digitalWrite(motorLeftDir_pin, HIGH);
  }
  else if (pwmLeftReq < 0) { // Left wheel reverse
    digitalWrite(motorLeftDir_pin, LOW);
  }
  else if (pwmLeftReq == 0 && pwmLeftOut == 0 ) { // Left wheel stop
    analogWrite(motorLeftPWM_pin, pwmLeftReq);
  }
  else { // Left wheel stop
    analogWrite(motorLeftPWM_pin, 0);
  }
 
  if (pwmRightReq > 0) { // Right wheel forward
    digitalWrite(motorRightDir_pin, HIGH);

  }
  else if(pwmRightReq < 0) { // Right wheel reverse
    digitalWrite(motorRightDir_pin, LOW);
    
  }
  else if (pwmRightReq == 0 && pwmRightOut == 0) { // Right wheel stop
    analogWrite(motorRightPWM_pin, pwmRightReq);
  }
  else { // Right wheel stop
    analogWrite(motorRightPWM_pin, 0); 
  }
  ...
  analogWrite(motorLeftPWM_pin, pwmLeftOut); 
  analogWrite(motorRightPWM_pin, pwmRightOut);
}
...
void setup() {
  ...
  pinMode(motorRightPWM_pin, OUTPUT);
  pinMode(motorRightDir_pin, OUTPUT);
  pinMode(motorLeftPWM_pin, OUTPUT);
  pinMode(motorLeftDir_pin, OUTPUT);
