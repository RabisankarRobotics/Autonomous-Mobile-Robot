...
//    float angular_z = cmd_vel_msg.angular.z;
    ...
    // Check if both linear and angular velocities are zero
//    if (cmd_vel_msg.linear.x == 0.0 && cmd_vel_msg.angular.z == 0.0) {
//        // Stop generating PWM signals
//        analogWrite(motor1PWM_pin, 0);
//        analogWrite(motor2PWM_pin, 0);
//        return;
//    }

    double left_wheel_velocity = cmd_vel_msg.linear.x - (cmd_vel_msg.angular.z * 0.41 / 2.0);
    double right_wheel_velocity = cmd_vel_msg.linear.x + (cmd_vel_msg.angular.z * 0.41 / 2.0);

//    if (left_wheel_velocity != 0.0 && right_wheel_velocity != 0.0) {

    int left_pwm = map_pwm(left_wheel_velocity, 0.0, 1.5, 0, 255);
    int right_pwm = map_pwm(right_wheel_velocity, 0.0, 1.5, 0, 255);
...
//    pwm_value = constrain(pwm_value, 0, 200);
    pwm_value = max(0, min(pwm_value, pwm_range_limit));
    return pwm_value;
}
